# C07 - op $73 fix: 8-cyc 2-byte RMW illegal (netlist=suite structure; A-model residue per sample)
...
- gate: B3 decision (fix list must include $73) + one-writer rule: C tasks
...
netlist=suite behaviour for op $73, per the adjudication.
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $73
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c07_pre/).
...
## Acceptance
- build/c07_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c07_pre/, build/c07_post/ + build/c07_post.md
