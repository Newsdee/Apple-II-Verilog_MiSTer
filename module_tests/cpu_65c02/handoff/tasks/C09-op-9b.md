# C09 - op $9b fix: netlist=suite 50/50 (structure + bus; A preserved by netlist+cores, suite modifies A)
...
- gate: B3 decision (fix list must include $9b) + one-writer rule: C tasks
...
netlist=suite behaviour for op $9b, per the adjudication.

## Expected silicon behaviour (from adjudication)
netlist=suite 50/50 (structure + bus; A preserved by netlist+cores, suite modifies A)
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $9b
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c09_pre/).
...
## Acceptance
- build/c09_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c09_pre/, build/c09_post/ + build/c09_post.md
