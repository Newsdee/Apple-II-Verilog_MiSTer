# C12 - op $f3 fix: 8-cyc 2-byte RMW illegal (netlist=suite structure; A-model residue per sample)
...
- gate: B3 decision (fix list must include $f3) + one-writer rule: C tasks
...
netlist=suite behaviour for op $f3, per the adjudication.

## Expected silicon behaviour (from adjudication)
8-cyc 2-byte RMW illegal (netlist=suite structure; A-model residue per sample)
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $f3
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c12_pre/).
...
## Acceptance
- build/c12_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c12_pre/, build/c12_post/ + build/c12_post.md
