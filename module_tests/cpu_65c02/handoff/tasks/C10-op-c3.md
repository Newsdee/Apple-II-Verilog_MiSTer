# C10 - op $c3 fix: netlist=suite 50/50 (structure + bus; A preserved by netlist+cores, suite modifies A)
...
- gate: B3 decision (fix list must include $c3) + one-writer rule: C tasks
...
netlist=suite behaviour for op $c3, per the adjudication.
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $c3
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c10_pre/).
...
## Acceptance
- build/c10_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c10_pre/, build/c10_post/ + build/c10_post.md
