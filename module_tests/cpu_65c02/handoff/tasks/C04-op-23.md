# C04 - op $23 fix: 8-cyc 2-byte RMW illegal (netlist=suite structure; A preserved by netlist+cores, suite modifies A)
...
- gate: B3 decision (fix list must include $23) + one-writer rule: C tasks
...
netlist=suite behaviour for op $23, per the adjudication.

## Expected silicon behaviour (from adjudication)
8-cyc 2-byte RMW illegal (netlist=suite structure; A preserved by netlist+cores, suite modifies A)
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $23
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c04_pre/).
...
## Acceptance
- build/c04_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c04_pre/, build/c04_post/ + build/c04_post.md
