# C11 - op $db fix: netlist=suite 50/50 (structure + bus; A preserved by netlist+cores, suite modifies A)
...
- gate: B3 decision (fix list must include $db) + one-writer rule: C tasks
...
netlist=suite behaviour for op $db, per the adjudication.
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $db
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c11_pre/).
...
## Acceptance
- build/c11_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c11_pre/, build/c11_post/ + build/c11_post.md
