# C05 - op $3b fix: 7-cyc 3-byte RMW illegal (netlist=suite structure; A preserved by netlist+cores, suite modifies A)
...
- gate: B3 decision (fix list must include $3b) + one-writer rule: C tasks
...
netlist=suite behaviour for op $3b, per the adjudication.

## Expected silicon behaviour (from adjudication)
7-cyc 3-byte RMW illegal (netlist=suite structure; A preserved by netlist+cores, suite modifies A)
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $3b
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c05_pre/).
...
## Acceptance
- build/c05_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c05_pre/, build/c05_post/ + build/c05_post.md
