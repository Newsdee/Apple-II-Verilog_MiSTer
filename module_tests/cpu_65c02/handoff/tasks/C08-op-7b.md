# C08 - op $7b fix: 7-cyc 3-byte RMW illegal (netlist=suite structure; A-model residue per sample)
...
- gate: B3 decision (fix list must include $7b) + one-writer rule: C tasks
...
netlist=suite behaviour for op $7b, per the adjudication.

## Expected silicon behaviour (from adjudication)
7-cyc 3-byte RMW illegal (netlist=suite structure; A-model residue per sample)
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $7b
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c08_pre/).
...
## Acceptance
- build/c08_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c08_pre/, build/c08_post/ + build/c08_post.md
