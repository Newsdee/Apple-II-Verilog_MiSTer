# C03 - op $7c fix: JMP (abs,X) 3-byte MOS (netlist=suite; cores decode 65C02 3-byte with different settle)
...
- gate: B3 decision (fix list must include $7c) + one-writer rule: C tasks
...
netlist=suite behaviour for op $7c, per the adjudication.

## Expected silicon behaviour (from adjudication)
JMP (abs,X) 3-byte MOS (netlist=suite; cores decode 65C02 3-byte with different settle)
...
- v2nmos: evidence/sweep_6502_v2nmos_results.txt rows for op $7c
...
- Capture the core's current 16-row trace for 3 sample idx values before
  touching RTL (pre-fix baseline, retained under build/c03_pre/).
...
## Acceptance
- build/c03_post.md: pre/post traces for the sample idx, sweep-slice
...
- build/c03_pre/, build/c03_post/ + build/c03_post.md
