...
--
-- Why: the real timing generator has no reset. On FPGA hardware and in
-- Verilator, power-on register values are defined (zero), so the HAL network
-- self-starts deterministically. In GHDL, an uninitialized std_logic is U, and
-- U propagates through not/xor forever (verified experimentally: PHI0 never
-- leaves U), so the golden simulation can never run without this shim.
--
-- The initial values below match Verilator power-on state exactly, so both
-- sides start from the same defined state; any divergence afterwards is a real
-- RTL difference, not an initialization artifact. No logic was changed.
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
--
...
entity timing_generator is
  ...
  port (
    ...
    SEGA        : buffer std_logic := '0';
    SEGB        : buffer std_logic := '0';
    SEGC        : buffer std_logic := '0';
    GR1         : buffer std_logic := '0';
    GR2         : buffer std_logic := '0';
    HBLANK         : out std_logic := '0';      -- Horizontal blanking
    VBLANK         : out std_logic := '0';      -- Vertical blanking
    WNDW_N         : out std_logic := '0';      -- Composite blanking
    LDPS_N         : out std_logic := '0'
  );
...
architecture rtl of timing_generator is
  ...
  signal COLOR_DELAY_N : std_logic := '0';
